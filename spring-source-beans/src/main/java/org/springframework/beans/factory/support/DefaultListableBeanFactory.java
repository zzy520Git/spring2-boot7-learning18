 ...
 *
 * 主要功能: TODO zzy
 * 默认实现：DefaultListableBeanFactory是Spring框架默认实现ConfigurableListableBeanFactory和BeanDefinitionRegistry接口的类。
 * 它是一个基于bean定义元数据的完整bean工厂实现，并且可以通过后置处理器进行扩展。
 * 1、典型使用场景
 * 注册bean定义：典型的使用场景是先注册所有的bean定义（可能是从bean定义文件中读取），然后再访问这些bean。因此，
 * 通过名称查找bean是一个低成本的操作，因为在本地的bean定义表中进行查找，并且操作的是预先解析的bean定义元数据对象。
 * 2、实现细节
 * 特定格式的读取器：通常，特定格式的bean定义读取器是单独实现的，而不是作为bean工厂的子类。例如，org.springframework.beans.factory.xml.XmlBeanDefinitionReader就是一个用于读取XML格式bean定义的读取器。
 * 3、替代实现
 * 静态bean工厂：如果你需要一个替代的ListableBeanFactory实现，可以查看StaticListableBeanFactory。这个类管理现有的bean实例，而不是根据bean定义创建新的bean实例。
 *
 * @author Rod Johnson
...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
    ...
    static {
        try {
            log.warn("DefaultListableBeanFactory start loading zzy");
     ...
     * 顺序排列的手动注册的单例（singleton）bean的名字。
     *
     * 在Spring框架中，单例bean是在整个应用上下文中只有一个实例的对象。通常情况下，这些bean是在配置文件（如XML文件或Java配置类）中定义的，或者通过某种方式（如注解）自动扫描并注册到Spring容器中的。
     *
     * 然而，有时候开发者需要在运行时手动注册一些单例bean，而不是通过配置文件或自动扫描。这种情况下的注册通常是通过ConfigurableBeanFactory接口的registerSingleton方法完成的。
     * 当手动注册多个单例bean时，这些bean的名字会被记录下来，并按照它们被注册的顺序存储在列表中。
     */
    ...
    public Object resolveDependency(DependencyDescriptor descriptor, @Nullable String requestingBeanName,
        ...
        if (Optional.class == descriptor.getDependencyType()) {
            ...
            throw new RuntimeException("zzy delete code!!!");
        } else {
