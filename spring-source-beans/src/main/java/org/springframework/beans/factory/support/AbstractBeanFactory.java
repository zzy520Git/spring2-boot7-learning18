 ...
 *
 * 这段描述介绍了 Spring 框架中 AbstractBeanFactory 类的基本信息及其提供的核心功能。
 * AbstractBeanFactory 是 BeanFactory 接口的一个抽象基类，提供了 ConfigurableBeanFactory SPI（服务提供者接口）的全部能力。
 *
 * 主要特点
 * 提供完整的 ConfigurableBeanFactory SPI 功能：
 * AbstractBeanFactory 实现了 ConfigurableBeanFactory 接口，这意味着它提供了 SPI 中定义的所有方法，包括但不限于单例管理、别名处理、工厂方法支持等。
 * 不假设 ListableBeanFactory：
 * 这个类并不依赖于 ListableBeanFactory 接口提供的列表功能，因此也可以作为那些从后端资源获取 Bean 定义的 BeanFactory 实现的基类，
 * 这些实现中访问 Bean 定义可能是一个昂贵的操作。
 * 提供多种核心功能：
 * 单例缓存：通过继承自 DefaultSingletonBeanRegistry 类，提供了单例 Bean 的缓存功能。
 * 单例/原型 Bean 的确定：能够区分单例（singleton）和原型（prototype）模式的 Bean。
 * 工厂方法支持：支持 FactoryBean，允许通过工厂方法创建复杂的 Bean 实例。
 * 别名支持：支持 Bean 的别名，使同一个 Bean 可以通过不同的名字引用。
 * 子 Bean 定义合并：能够合并子 Bean 定义，支持更复杂的 Bean 结构。
 * Bean 销毁：支持 Bean 的销毁操作，包括实现了 DisposableBean 接口的对象和自定义销毁方法。
 * 管理 BeanFactory 层次结构：
 * 通过实现 HierarchicalBeanFactory 接口，AbstractBeanFactory 支持 BeanFactory 的层次结构，允许在未知 Bean 的情况下委托给父级 BeanFactory 处理。
 * 主要模板方法
 * AbstractBeanFactory 类定义了几个主要的模板方法，子类需要实现这些方法以完成特定的功能：
 *
 * getBeanDefinition：
 * 获取给定 Bean 名称的 Bean 定义。这个方法由子类实现，以从存储中加载 Bean 定义。
 * createBean：
 * 根据给定的 Bean 定义创建 Bean 实例。这个方法也是由子类实现，以完成 Bean 的实例化工作。
 * 子类实现
 * 默认的实现可以找到两个具体的类中：
 *
 * DefaultListableBeanFactory：
 * 提供了 ListableBeanFactory 接口的实现，支持列出所有 Bean 的名称，并且提供了默认的 getBeanDefinition 方法实现。
 * AbstractAutowireCapableBeanFactory：
 * 继承自 AbstractBeanFactory，提供了自动装配（autowiring）的能力，并且提供了默认的 createBean 方法实现。
 *
 * @author Rod Johnson
...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
     ...
     *                      not for actual use 判断是否用于类型检查，而不是实际使用(getBean)
     * @return an instance of the bean
     * @throws BeansException if the bean could not be created
     */
            ...
            try {
                ...
                if (dependsOn != null) {
                    for (String dep : dependsOn) {
                        ...
                        // 注册依赖，dep是beanName的依赖，亦即beanName依赖dep
                ...
                if (mbd.isSingleton()) {
                    ...
                    // 判断是否FactoryBean等操作，是则要执行getObject方法
     ...
     * 预测 Bean 类型：
     * 这个方法的作用是预测一个 Bean 的最终类型。这里的“最终类型”指的是 Bean 实例在经过所有必要的处理（如依赖注入、后置处理器等）之后的真实类型。
     * 调用者：
     * 该方法是由 getType 和 isTypeMatch 方法调用的。getType 方法用于获取 Bean 的类型，而 isTypeMatch 方法用于判断 Bean 是否匹配指定的类型。
     * 不处理 FactoryBeans：
     * 由于该方法只关注 Bean 的原始类型，因此不需要特别处理 FactoryBean。FactoryBean 是 Spring 中的一种特殊 Bean，它可以生产其他 Bean。
     * 简单的实现：
     * 这段描述提到的实现比较简单，不能处理工厂方法和 InstantiationAwareBeanPostProcessor（实例化感知的 Bean 后置处理器）。
     * 这意味着它只能正确预测标准 Bean 的类型，而对于通过工厂方法创建的 Bean 或者通过后置处理器修改了类型的 Bean，则可能无法正确预测。
     * 子类扩展：
     * 为了支持更复杂的类型检测，建议在子类中重写这个方法以提供更精确的类型预测。
     *
     * @param beanName     the name of the bean
    ...
    protected void markBeanAsCreated(String beanName) {
        if (!this.alreadyCreated.contains(beanName)) {
            synchronized (this.mergedBeanDefinitions) {
                if (!this.alreadyCreated.contains(beanName)) {
                    //TODO Let the bean definition get re-merged now that we're actually creating
    ...
    protected Object getObjectForBeanInstance(
        ...
        if (mbd != null) {
            ...
            // 获取缓存的FactoryBean生产的Object Bean only once
    ...
    static class BeanPostProcessorCache {
        ...
        /**
         * TODO 可以关注下，这几个后置处理器是做什么的
         * 包含：AutowiredAnnotationBeanPostProcessor、AOP
         */
