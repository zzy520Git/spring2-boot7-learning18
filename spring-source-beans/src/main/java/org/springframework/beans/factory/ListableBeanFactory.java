 ...
 *
 * 主要功能：TODO
 * 枚举所有bean实例：ListableBeanFactory接口的实现可以列出所有已定义的bean实例，而不是在客户端请求时逐个查找。
 * 实现细节
 * 如果实现了HierarchicalBeanFactory，那么返回值不会考虑bean工厂的层级关系，仅包含当前工厂中定义的bean。如果需要考虑父级工厂中的bean，可以使用BeanFactoryUtils辅助类。
 * 接口中的方法主要关注当前工厂中的bean定义。除了getBeanNamesForType和getBeansOfType会检查手动注册的单例外，其他方法会忽略通过ConfigurableBeanFactory的registerSingleton方法注册的单例bean。当然，BeanFactory的getBean方法仍然允许透明地访问这些特殊bean。
 * 注意事项
 * 注释特别指出，除了getBeanDefinitionCount和containsBeanDefinition方法外，该接口中的其他方法并不是设计来频繁调用的，因此实现可能较慢。
 *
 * @author Rod Johnson
