 ...
 * interface.
 * 这段描述介绍了一个基类 AbstractBeanDefinition，它是具体的、完整的 BeanDefinition 类的基类，
 * 提取了 GenericBeanDefinition、RootBeanDefinition 和 ChildBeanDefinition 的共同属性。
 * AbstractBeanDefinition 是一个抽象类，为子类提供了通用的属性和行为。
 *
 * 主要特点
 * 通用属性：
 * AbstractBeanDefinition 抽象类为具体的 BeanDefinition 子类提供了一组通用的属性，比如 autowireMode、dependencyCheck、lazyInit 等。
 * 自动装配模式：
 * AbstractBeanDefinition 中定义了自动装配模式（autowireMode）相关的常量，这些常量与 AutowireCapableBeanFactory 接口中定义的常量相匹配。
 * 这允许开发者指定如何自动装配 Bean 的依赖项。
 * 依赖检查：
 * dependencyCheck 属性用于指定在 Bean 初始化之前是否检查依赖项的存在。
 * 延迟初始化：
 * lazyInit 属性决定了 Bean 是否延迟初始化。如果设置为 true，则 Bean 只有在第一次被请求时才会被初始化。
 * 常见属性
 * AbstractBeanDefinition 类中的一些常见属性包括：
 *
 * String beanClassName：Bean 类的完全限定名。
 * Class<?> resolvedBeanClass：解析后的 Bean 类。
 * int autowireMode：自动装配模式。
 * int dependencyCheck：依赖检查模式。
 * boolean lazyInit：是否延迟初始化。
 * String initMethodName：初始化方法名。
 * String destroyMethodName：销毁方法名。
 * boolean singleton：是否为单例。
 * boolean prototype：是否为原型。
 * 常量
 * AbstractBeanDefinition 中定义了与 AutowireCapableBeanFactory 接口中相同的常量，用于指定自动装配模式：
 *
 * AUTOWIRE_NO：不自动装配。
 * AUTOWIRE_BY_NAME：按名称自动装配。
 * AUTOWIRE_BY_TYPE：按类型自动装配。
 * AUTOWIRE_CONSTRUCTOR：构造函数自动装配。
 * AUTOWIRE_AUTODETECT：自动检测自动装配模式。
 *
 * @author Rod Johnson
...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
     ...
     * 这段描述介绍了一个用于替代声明性工厂方法的回调机制，用于创建 Bean 的实例。通过设置这样一个回调，
     * 它可以覆盖任何其他的构造函数或工厂方法元数据。然而，Bean 属性填充和潜在的注解驱动的注入仍然会按照常规方式应用。
     *
     * 回调机制的作用
     * 在 Spring 框架中，通常有两种方式来创建 Bean 的实例：
     *
     * 构造函数注入：通过构造函数来初始化 Bean。
     * 工厂方法注入：通过定义一个工厂类中的静态方法来创建 Bean。
     * 然而，有时候你可能希望使用一种更加灵活的方式来创建 Bean 的实例，这时候就可以使用回调机制。
     * 这种方式允许你在运行时决定如何创建 Bean 的实例，而不是在配置文件或注解中预先定义。
     *
     ...
     * 这段描述解释了如何在 Spring 框架中配置是否允许访问非公共（non-public）的构造函数和方法。
     * 这个设置默认是允许的（true），但如果只想允许访问公共（public）成员，可以将其设置为 false。
     * 这个设置会影响构造函数解析、工厂方法解析以及初始化（init）和销毁（destroy）方法的访问。需要注意的是，Bean 的属性访问器必须总是公共的，并且不受此设置的影响。
     *
     * 设置说明
     * 默认设置：默认情况下，这个设置是 true，意味着 Spring 容器在解析构造函数、工厂方法以及初始化和销毁方法时，可以访问非公共成员。
     * 仅限公共成员：如果将这个设置改为 false，那么 Spring 容器只会访问公共成员，不允许访问非公共成员。
     * 设置的作用范围
     * 这个设置主要影响以下几个方面：
     *
     * 构造函数解析：当 Spring 容器需要通过构造函数来创建 Bean 实例时，它会根据这个设置来决定是否可以访问非公共构造函数。
     * 工厂方法解析：当使用工厂方法来创建 Bean 实例时，Spring 容器也会根据这个设置来决定是否可以访问非公共工厂方法。
     * 初始化和销毁方法：同样地，当配置了初始化或销毁方法时，Spring 容器也需要根据这个设置来决定是否可以访问非公共的初始化或销毁方法。
     * 对 Bean 属性的影响
     * 需要注意的是，无论这个设置是什么，Bean 的属性访问器（getter 和 setter 方法）必须总是公共的，并且不受这个设置的影响。
     */
