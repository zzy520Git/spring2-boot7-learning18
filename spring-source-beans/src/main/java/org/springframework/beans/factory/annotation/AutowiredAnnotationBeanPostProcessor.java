 ...
 *
 * BeanPostProcessor 实现，用于自动装配带有注解的字段、setter 方法和任意配置方法。这些需要注入的成员通过注解检测：默认情况下，Spring 的 @Autowired 和 @Value 注解。
 * TODO
 * 此外，还支持 JSR-330 的 @Inject 注解（如果可用），作为 Spring 自身 @Autowired 注解的直接替代品。
 * 1、注解构造器
 * 只有一个给定的 bean 类的构造器可以声明此注解，并且将 required 属性设置为 true，这表明是 <i>该</i> 构造器将在作为 Spring bean 使用时进行自动装配。
 * 此外，如果 required 属性设置为 true，则只能有一个构造器标注 @Autowired。
 * 如果多个 <i>非必需</i> 构造器声明了此注解，它们将被视为自动装配的候选构造器。
 * 将选择依赖项最多且可以通过 Spring 容器中的匹配 bean 满足的构造器。
 * 如果没有候选构造器可以满足，则使用主/默认构造器（如果存在）。
 * 如果一个类只声明了一个构造器，则始终使用该构造器，即使没有标注。
 * 一个标注了 @Autowired 的构造器不必是公共的。
 * 2、注解字段，父类的@Autowired也会注入
 * 字段在 bean 构造之后立即注入，在任何配置方法被调用之前。
 * 这样的配置字段不必是公共的。
 * 3、注解方法
 * 配置方法可以有任意名称和任意数量的参数；每个参数都将与 Spring 容器中的匹配 bean 进行自动装配。
 * bean 属性 setter 方法实际上是此类通用配置方法的一种特殊情况。
 * 配置方法不必是公共的。
 * 4、注解配置与 XML 配置
 * 默认的 AutowiredAnnotationBeanPostProcessor 将通过 "context:annotation-config" 和 "context:component-scan" XML 标签注册。
 * 如果你打算指定一个自定义的 AutowiredAnnotationBeanPostProcessor bean 定义，请移除或关闭默认的注解配置。
 * <b>注意：</b> 注解注入将在 XML 注入 <i>之前</i> 进行；因此，对于通过两种方式配置的属性，XML 配置将覆盖注解配置。
 * 5、@Lookup 方法
 * 除了上述常规注入点之外，此后处理器还处理 Spring 的 @Lookup 注解，该注解标识了运行时由容器替换的查找方法。
 * 这本质上是一个类型安全的 getBean(Class, args) 和 getBean(String, args) 版本。
 * 详见 @Lookup 的 Javadoc 以获取详细信息。
 *
...
public class AutowiredAnnotationBeanPostProcessor implements SmartInstantiationAwareBeanPostProcessor,
    ...
    public AutowiredAnnotationBeanPostProcessor() {
        // TODO 用于支持解析这三种 注解：Autowired、Value、Inject
