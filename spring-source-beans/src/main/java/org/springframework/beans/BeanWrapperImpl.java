 ...
 *
 * 这段描述介绍了 Spring 框架中 BeanWrapperImpl 类的基本信息，这是 BeanWrapper 接口的一个默认实现。BeanWrapper 接口和其实现类主要用于对 JavaBean 进行包装，
 * 以便更容易地访问和修改其属性。BeanWrapperImpl 作为默认实现，通常足以应对大多数常见的使用场景。
 *
 * 主要特性
 * 高效缓存：
 * BeanWrapperImpl 缓存了自省（introspection）的结果，以提高性能。这意味着在多次访问相同类型的 Bean 属性时，不会重复进行自省操作，从而提高了效率。
 * 自动注册默认的属性编辑器：
 * 自动注册了 org.springframework.beans.propertyeditors 包中的默认属性编辑器，
 * 这些编辑器在 java.beans.PropertyEditor 标准编辑器的基础上提供了额外的支持。这些默认编辑器适用于大多数标准的数据类型转换场景。
 * 注册自定义属性编辑器：
 * 应用程序可以通过调用 registerCustomEditor 方法来为特定的属性注册自定义的 PropertyEditor。
 * 这些编辑器不是全局共享的，而是针对特定的 BeanWrapperImpl 实例。
 * 内部类：
 * 自 Spring 2.5 版本开始，BeanWrapperImpl 主要作为一个内部类使用，虽然它是公开的，以便其他框架包可以访问。
 * 对于标准应用程序，建议使用 PropertyAccessorFactory 的 forBeanPropertyAccess 工厂方法来创建 BeanWrapper 实例。
 *
 * @author Rod Johnson
