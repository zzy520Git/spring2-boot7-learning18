 ...
 * 特点：TODO
 * 单一内部 BeanFactory：
 * GenericApplicationContext 持有一个内部的 DefaultListableBeanFactory 实例，而不是像其他 ApplicationContext 实现那样在每次刷新时创建一个新的 BeanFactory 实例。
 * 非特定格式的 Bean 定义：
 * 该实现不对 Bean 定义的格式做出假设，因此可以非常灵活地注册不同格式的 Bean 定义。
 * 实现 BeanDefinitionRegistry：
 * 通过实现 BeanDefinitionRegistry 接口，允许在上下文刷新前注册 Bean 定义。
 * 刷新上下文：
 * 通过调用 refresh() 方法初始化已注册的 Bean，处理 ApplicationContextAware 接口实现类，自动检测 BeanFactoryPostProcessor 等。
 * 仅限一次的刷新：
 * refresh() 方法只能调用一次，这意味着一旦上下文被刷新，就不能再次刷新。
 *
