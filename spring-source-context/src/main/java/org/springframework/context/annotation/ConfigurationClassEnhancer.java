...
class ConfigurationClassEnhancer {
     ...
     *
     * 这段描述介绍了一个标记接口，该接口用于所有由 CGLIB 生成的 @Configuration 类的子类。
     * 这个接口的主要目的是为了实现 ConfigurationClassEnhancer#enhance 方法的幂等性行为，
     * 也就是说，当增强器检查候选类是否已经是该接口的实现类时，就可以判断该类是否已经被增强过。
     *
     * 主要用途
     * 幂等性行为：
     * 当使用 ConfigurationClassEnhancer#enhance 方法增强 @Configuration 类时，如果目标类已经是该接口的实现类，那么就不需要再次增强。这可以防止重复增强同一个类。
     * BeanFactoryAware 扩展：
     * 所有增强过的 @Configuration 类都实现了 BeanFactoryAware 接口，这意味着它们可以获得对创建它们的 BeanFactory 的引用。
     * 这是因为 @Configuration 类可能需要访问 BeanFactory 来执行一些操作，比如在 @Bean 方法中创建和配置其他 Bean。
     * 接口定义
     * 这个接口本身通常不会包含任何方法，因为它是作为一个标记接口来使用的。它主要的作用是为了标识某个类已经被增强过了，并且可以让增强后的类获得对 BeanFactory 的引用。
     */
